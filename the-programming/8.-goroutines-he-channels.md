# 8. Goroutines和Channels（一）

## 8.1. Goroutines

每一个并发执行的单元叫做一个goroutine。程序启动时，主函数在一个单独的goroutine中执行，叫做main goroutine。
...
time.Time.Format 方法提供了格式化日期的功能。模板限定为 Mon Jan 2 03:04:05PM 2006 UTC-0700 。（可以按1234567的方式来记忆）
